...
3. **El editor viejo `pages/02-layout-editor/` está descartado y aprobado el reemplazo (2026-09-17).** No se le agrega nada nuevo; su README tiene la nota de descarte. Se conserva en el repo como referencia histórica, no se borra.
...
**2026-09-17 — Aprobado para producción.** El dueño confirmó: "este nuevo editor es el real, el anterior se descarta y este es el aprobado que se usará en producción". `02-layout-editor` queda marcado como **descartado** (nota al pie en su README, fila del `src/README.md` y del mapa de pantallas) pero **no se borra** el código — el dueño prefirió dejarlo en el repo como referencia histórica en vez de eliminarlo. Ningún agente debe seguir desarrollando ahí.

Primer commit del editor nuevo hecho el mismo día (`feat(editor): reconstruccion completa del editor de plantillas`), tocando solo los archivos de esta tarea, con `.gitignore` nuevo para `src/fonts/` (fuentes personales del dueño, sin licencia de redistribución) y el material de referencia local (`pantallas dslrbooth/`, `ui inspo/`, `memoria_trabajo_para_ias.txt`) que el `CLAUDE.md` del repo pide mantener fuera del historial. Con su OK arranca la **Fase 4 (pulido)**: modo claro y switch, auditoría contra DESIGN.md, motion, revisión del redimensionar rotado, actualizar mapa/README, decidir el retiro de `02-layout-editor`. Recién después arrancar **Fase 2 (tipografía)**: crear `tipografia.js`, ampliar el bloque "Texto" del inspector y la barra contextual, y agregar la carga de fuentes locales guardadas en `plantilla.recursos.fuentes`.
